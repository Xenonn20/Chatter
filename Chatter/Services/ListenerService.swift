...
class ListenerService {
    ...
    func messagesObserve(chat: MChat, completion: @escaping (Result<MMessage, Error>) -> Void) -> ListenerRegistration? {
           let ref = usersRef.document(currentUserId).collection("activeChat").document(chat.friendId).collection("messages")
           let messagesListener = ref.addSnapshotListener { (querySnapshot, error) in
               ...
               snapshot.documentChanges.forEach { (diff) in
                   guard let message = MMessage(documents: diff.document) else { return }
                   switch diff.type {
                   case .added:
